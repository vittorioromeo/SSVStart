...
namespace ssvs
{
    namespace Input
    {
            ...
            Manager& mManager, InputState& mInputState, Mode mMode) const
        {
            ...
            // If the combo is exclusive, check if its keys/btns were pressed
            // previously in this frame
            if(mMode == Mode::Exclusive)
            {
                if((mManager.processedInput.getKeys() & keys).any())
                    return false;
                if((mManager.processedInput.getBtns() & btns).any())
                    return false;
            }

            // Combo will return true: add its keys/btns to this frame's
            // processed
            // inputs
