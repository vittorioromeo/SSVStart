...
namespace ssvs
{
	namespace BTR
	{
		namespace Impl
		{
			...
			template<typename T> inline auto& BTRChunk::in(Ptr<Chunk>& mPtr, T&& mStr)
			{
			...
			template<typename T> inline auto& BTRChunk::mk(Ptr<Chunk>& mPtr, T&& mStr)	{ in(mPtr, FWD(mStr)); return *this; }
			inline auto& BTRChunk::mk(Ptr<Chunk>& mPtr)									{ in(mPtr, ""); return *this; }
			...
			template<typename T, typename... TArgs> inline auto& BTRChunk::eff(Ptr<T>& mPtr, TArgs&&... mArgs)	{ mPtr = &EffectHelper<T>::mk(*this, FWD(mArgs)...); return *this; }
