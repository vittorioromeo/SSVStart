...
namespace ssvs
{
	...
	{
		template<typename TR, typename... TArgs> inline auto& load(TR& mRH, const std::string& mId, TArgs&&... mArgs)
		{
			...
			template<typename T> inline void checkMissing(T& mRH, const std::string& mId) { SSVU_ASSERT(mRH.has(mId)); }
		#else
			template<typename T> inline void checkMissing(T&, const std::string&) { }
		#endif
	};
	...
	struct RHPolicyDefault
	{
		...
		template<typename T> inline void checkMissing(T& mRH, const std::string& mId)
		{
