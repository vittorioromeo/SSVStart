...
namespace ssvs
{
    ...
    public:
        inline Camera(GameWindow& mGameWindow, const sf::View& mView)
            : gameWindow(mGameWindow), renderWindow(gameWindow), view{mView}
        {
        }

        inline Camera(GameWindow& mGameWindow, const Vec2f& mCenter,
            float mZoomFactor = 1.f)
            : gameWindow(mGameWindow), renderWindow(gameWindow),
              view{mCenter, {gameWindow.getWidth() / mZoomFactor,
                                gameWindow.getHeight() / mZoomFactor}}
        {
            SSVU_ASSERT(mZoomFactor != 0);
        }

        inline Camera(GameWindow& mGameWindow, float mZoomFactor = 1.f)
            : gameWindow(mGameWindow), renderWindow(gameWindow),
        ...
        inline void apply()
        {
            if(mustRecompute)
            {
                ...
                computedView.setSize(
                    toNum<T>(computedView.getSize().x * skew.x),
        ...
        inline void pan(const Vec2f& mVec)
        {
            nextPan += mVec;
            invalid = true;
        }
        inline void pan(float mX, float mY)
        {
            nextPan.x += mX;
            nextPan.y += mY;
            invalid = true;
        }
        inline void zoomIn(float mFactor)
        {
            nextZoomFactor /= mFactor;
            invalid = true;
        }
        inline void zoomOut(float mFactor)
        {
            nextZoomFactor *= mFactor;
            invalid = true;
        }
        inline void turn(float mDeg)
        {
            nextRotation += mDeg;
            invalid = true;
        }
        ...
        inline void setView(const sf::View& mView) noexcept
        {
            view = mView;
            mustRecompute = true;
        }
        inline void setRotation(float mDeg) noexcept
        {
            view.setRotation(mDeg);
            mustRecompute = true;
        }
        inline void setSkew(const Vec2f& mSkew) noexcept
        {
            skew = mSkew;
            mustRecompute = true;
        }
        ...
        inline void setCenter(const Vec2f& mPosition) noexcept
        {
        ...
        inline void setNextZoomFactor(float mFactor) noexcept
        {
            nextZoomFactor = mFactor;
            invalid = true;
        }
        ...
        inline const auto& getCenter() const noexcept
        {
            return view.getCenter();
        }
        inline float getNextZoomFactor() const noexcept
        {
        ...
        inline auto getConvertedCoords(const Vec2i& mPos) const
        {
